...
class Sum_Decoder(nn.Module):   #decoder summarizaer
    def __init__(self):
        super(Sum_Decoder, self).__init__()
    ...
    def forward(self, y_t, s_t, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, sum_temporal_srcs, prev_s):
        x = self.x_context(T.cat([y_t, ct_e], dim=1))
        s_t = self.lstm(x, s_t)

        dec_h, dec_c = s_t
        st_hat = T.cat([dec_h, dec_c], dim=1)

        ct_e, attn_dist, sum_temporal_srcs = self.enc_attention(st_hat, enc_out, enc_padding_mask, sum_temporal_srcs)   #Encoder Attention
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.sum_decoder = get_cuda(self.sum_decoder)
