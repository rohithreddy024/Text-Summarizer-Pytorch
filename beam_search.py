...
class Beam(object):
    ...
    def get_best(self):
        best_tokens = self.tokens[0].cpu().numpy().tolist()          #Since beams are always in sorted (descending) order, 1st beam is the best beam
        try:
            end_idx = best_tokens.index(self.end_id)
        except ValueError:
            end_idx = len(best_tokens)
        best_tokens = best_tokens[1:end_idx]
        return best_tokens

def beam_search_on_batch(enc_hid, enc_out, enc_padding_mask, c_t_1, extra_zeros, enc_batch_extend_vocab, model, start_id, end_id, unk_id):

    batch_size = len(enc_hid[0])
    beams = [Beam(start_id, end_id, unk_id, (enc_hid[0][i], enc_hid[1][i]), c_t_1[i]) for i in range(batch_size)]   #For each example in batch, create Beam object
    beam_idx = T.LongTensor(list(range(batch_size)))                    #Index of beams that are active, i.e: didn't generate [STOP] yet
    n_rem = batch_size                                                  #Number of examples in batch that didn't generate [STOP] yet
    ...
    for t in range(config.max_dec_steps):
        ...
        y_t_1 = model.embeds(y_t_1)                                                 #rem*beam, n_emb
        ...
        final_dist, (dec_h, dec_c), c_t_1, sum_temporal_srcs, prev_s = model.sum_decoder(y_t_1, s_t_1, enc_out_beam, enc_pad_mask_beam, c_t_1, extra_zeros_beam, enc_extend_vocab_beam, sum_temporal_srcs, prev_s)
        ...
        c_t_1 = c_t_1.view(n_rem, config.beam_size, -1)                                 #rem, beam, 2*n_hid

        if sum_temporal_srcs is not None:
            sum_temporal_srcs = sum_temporal_srcs.view(n_rem, config.beam_size, -1)     #rem, beam, n_seq

        if prev_s is not None:
            prev_s = prev_s.view(n_rem, config.beam_size, -1, config.hidden_dim)        #rem, beam, t+1
            assert prev_s.size(2) == t+1
        ...
        for i in range(n_rem):
            ...
            temporal_srcs_i = prev_s_i = None
            if sum_temporal_srcs is not None:
                temporal_srcs_i = sum_temporal_srcs[i]  #beam, n_seq
            if prev_s is not None:
                prev_s_i = prev_s[i]                    #beam, t, n_hid
            beam.advance(final_dist[i], (dec_h[i], dec_c[i]), c_t_1[i], temporal_srcs_i, prev_s_i)
            if beam.done == False:
                active.append(b)
